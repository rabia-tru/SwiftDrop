...
import 'rider_splash_screen.dart';
import 'register_screen.dart';

/// SwiftDrop Rider Login — Theme-aware
...
class _LoginScreenState extends State<LoginScreen>
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (!mounted) return;
      // Route through the rider splash into MainNavigation (5-tab bottom
      // nav) — the bare HomeScreen left the rider stranded without tabs.
      Navigator.of(context).pushReplacement(
        PageRouteBuilder(
          pageBuilder: (context, animation, secondaryAnimation) =>
              const RiderSplashScreen(),
  ...
  Widget _buildLoginForm(Color textColor, Color subTextColor, Color inputBg) {
    return Container(
      ...
      children: [
        const Text('Rider Login', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
  ...
  Widget _buildField({required String label, required Widget child, Widget? labelRight, Color? textColor}) {
    return Column(
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(
                      _codeSent ? Icons.lock_open_rounded : Icons.lock_reset_rounded,
                          ...
                          onPressed: _sending ? null : () async {
                            if (!_codeSent) {
                              ...
                              if (emailController.text.trim().isEmpty) {
                                setDialogState(() => _error = 'Please enter your email');
                                return;
                              }
                              ...
                              try {
                                ...
                                setDialogState(() { _error = ErrorHelper.getMessage(e); _sending = false; });
                              }
                            } else {
                              // Step 2: Reset password
                              if (codeController.text.length != 6) {
                                setDialogState(() => _error = 'Please enter the 6-digit code');
                                return;
                              }
                              if (newPasswordController.text.length < 6) {
                                setDialogState(() => _error = 'Password must be at least 6 characters');
                                return;
                              }
                              setDialogState(() { _sending = true; _error = null; });
                              try {
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.shopping_bag, size: 16, color: subTextColor),
              const SizedBox(width: 4),
              Text('Switch to Customer Login', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: subTextColor)),
            ],
          ),
        ),
      ],
